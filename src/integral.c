...
static Tree* integral_sub2(Tree* u, map* v, const char* x)
{
	...
	if (u->tok_type == NEGATIF)
		return join(integral_sub2(u->tleft, v, x), NULL, u->value);
	if (u->gtype == OPERAT)
		return join(integral_sub2(u->tleft, v, x), integral_sub2(u->tright, v, x), u->value);
	return clone(u);
}

static Tree* integral_sub(Tree* u, map* v, const char* x)
{
	if ((*v) != NULL)
	{
		mapCell* tmp = (*v)->begin;
...
static Tree* add_form(Tree* u, const char* x, map* L)
{
	if (ispoly(u, x))
	{
		...
		if (coefs->length == 2)
		{
			...
			*L = push_back_map_if(*L, b, coefs->end->tr);
...
static Tree* integral_table(Tree* f, const char* x)
{
	...
	if (sol != NULL)
	{
		...
		Tree* ret = integral_sub(sol, &Li, x);
...
Tree* integrals_by_part(Tree* f, const char* x)
{
	if (f->tok_type == DIVID)
	{
		...
		if (v->tok_type == PROD)
		{
			Tree* w = v->tright;
			v = v->tleft;
			g = join(join(clone(u), clone(v), fnc[DIVID].ex), join(new_tree("1"), clone(w), fnc[DIVID].ex), fnc[PROD].ex);
		}
		else
	...
	if (f->tok_type == POW)
	{
		...
		if (v->tok_type == NEGATIF)
		{
			Tree* g = join(new_tree("1"), (!strcmp(v->tleft->value, "1")) ? clone(u) : join(clone(u), clone(v->tleft), fnc[POW].ex), fnc[DIVID].ex);
			Tree* intg = integrals_by_part(g, x);
			clean_tree(g);
			return intg;
		}
	...
	{
		if (f->tok_type == LN_F || f->tok_type == ASIN_F || f->tok_type == ACOS_F || f->tok_type == SIN_F || f->tok_type == COS_F || f->tok_type == COSH_F ||
			f->tok_type == SINH_F || f->tok_type == EXP_F)
		{
