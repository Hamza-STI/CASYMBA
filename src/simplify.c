...
bool greater(const char* a, const char* b)
{
	...
	int len_c = (c == NULL) ? 0 : dim(c), len_d = (d == NULL) ? 0 : dim(d);
	int len_a = dim(a) - len_c, len_b = dim(b) - len_d;
...
{
	if (dim(a) == new_size_int + new_size_dec)
		return strdup(a);
...
char* sub(const char* left, const char* right, int* sign)
{
	...
	int len_c = (c == NULL) ? 0 : dim(c), len_d = (d == NULL) ? 0 : dim(d);
	int len_a = dim(left) - len_c, len_b = dim(right) - len_d;
...
{
	char* c = strchr(left, '.'), * d = strchr(right, '.');
	int len_a = dim(left), len_b = dim(right), len_c = (c == NULL) ? 0 : dim(c) - 1, len_d = (d == NULL) ? 0 : dim(d) - 1;
...
{
	int len_a = dim(num), len_b = dim(denom), pos = 0, p = 0;
...
char* divid(const char* num, const char* denom)
{
	...
	int len_c = (c == NULL) ? 0 : dim(c) - 1, len_d = (d == NULL) ? 0 : dim(d) - 1;
	int len_a = dim(num), len_b = dim(denom), pos = 0, prec = 0, p = 0;
	...
	while (prec < 15)
	{
		...
		if ((pos < len_a && new_a[pos] == '.') || pos == len_a)
		{
			...
			prec++;
		tmp[dim(tmp)] = (pos >= len_a) ? '0' : new_a[pos];
		pos++;
	}
